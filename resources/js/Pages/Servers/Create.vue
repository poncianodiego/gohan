...
const form = useForm({
  ...
  region: 'nyc1',
  size: 's-2vcpu-4gb',
  cloud_credential_id: props.credentials[0]?.id || '',
  php_version: '8.3',
    ...
    <div class="pb-20">
      ...
      <div class="max-w-lg mx-auto px-4 py-4">
        <div v-if="!credentials.length" class="text-center py-12">
          ...
          <a href="/settings" class="text-primary hover:underline">Go to Settings</a>
        </div>

        <form v-else @submit.prevent="submit" class="space-y-4">
          <div v-if="form.errors.name" class="bg-danger/10 text-danger text-sm p-3 rounded-lg">
            {{ form.errors.name }}
          </div>

          <div>
            <label class="block text-sm text-text-muted mb-1">Server Name</label>
            <input v-model="form.name" type="text" required class="w-full px-4 py-3 bg-surface-light border border-surface-lighter rounded-lg text-text focus:outline-none focus:border-primary" />
          </div>

          <div>
            <label class="block text-sm text-text-muted mb-1">Cloud Credential</label>
            <select v-model="form.cloud_credential_id" class="w-full px-4 py-3 bg-surface-light border border-surface-lighter rounded-lg text-text focus:outline-none focus:border-primary">
          ...
          <div>
            ...
            <select v-model="form.region" class="w-full px-4 py-3 bg-surface-light border border-surface-lighter rounded-lg text-text focus:outline-none focus:border-primary">
          ...
          <button type="button" @click="showAdvanced = !showAdvanced" class="flex items-center text-sm text-text-muted hover:text-text">
          ...
          <div v-if="showAdvanced" class="space-y-4 pl-2 border-l-2 border-surface-lighter">
            <div>
              ...
              <select v-model="form.php_version" class="w-full px-4 py-3 bg-surface-light border border-surface-lighter rounded-lg text-text focus:outline-none focus:border-primary">
            ...
            <div>
              ...
              <select v-model="form.database_type" class="w-full px-4 py-3 bg-surface-light border border-surface-lighter rounded-lg text-text focus:outline-none focus:border-primary">
